...
def run_pipeline(options: PipelineOptions) -> PipelineResult:
    ...
    try:
        ...
        store.append_event(run, "forecast_started", questions=len(questions))
        ...
        store.append_event(run, "forecast_completed", records=len(records))
        ...
        store.append_event(run, "train_backtest_completed", total_evaluations=train_payload["total_evaluations"])
        ...
        store.append_event(run, "run_failed", error=str(exc))

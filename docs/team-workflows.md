...
> **Released sandbox note:** `xrtm==0.8.5` now includes a bounded
...
Released `xrtm runs export` supports both JSON and CSV in `0.8.5`:
...
**Current status:** User attribution and built-in multi-user control-plane features are still not in XRTM v0.8.5. Teams must use the patterns documented above.

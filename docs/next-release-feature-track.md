...
> The published baseline is `xrtm==0.8.6`: guided onboarding helpers,
...
> compatibility, and the bounded provider-free playground are on the released
> surface.
>
> This release keeps the runtime claim provider-free unless separate real-runtime
> validation widens it. The published `0.8.6` baseline does not claim a

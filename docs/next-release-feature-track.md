...
> This page tracks conveniences that are intentionally **not** part of the published `xrtm==0.3.2` surface. Guided onboarding helpers, latest-run shortcuts, and CSV export graduated in `0.3.1`; this page now tracks what remains unreleased.
...
| --- | --- | --- | --- | --- | --- |
| Guided onboarding helpers | `xrtm start`, `xrtm profile starter` | **`shipped`** | `0.3.1` | These wrappers are now part of the published provider-free first-success path. | Keep clean-install proof and release-pinned onboarding docs in lockstep with the published package and `xrtm.org` mirror. |
| Latest-run shortcuts | `latest` run ref plus `--latest` on inspection/report flows | **`shipped`** | `0.3.1` | These shortcuts are now part of the published operator ergonomics surface over canonical run directories. | Keep release smoke covering `runs show latest`, `artifacts inspect --latest`, and `report html --latest`. |
| Spreadsheet export | `xrtm runs export --format csv` | **`shipped`** | `0.3.1` | CSV export is now part of the published top-level product surface, with JSON still documented as the full-fidelity bundle. | Keep JSON/CSV docs aligned with release packaging and release-command contract checks. |

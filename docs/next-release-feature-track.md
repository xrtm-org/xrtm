...
> The published baseline is `xrtm==0.8.8`: guided onboarding helpers,
> latest-run shortcuts, CSV export, the shared CLI/WebUI workflow-authoring loop,
> the local WebUI Hub/Studio/Playground/Observatory spine, `/workbench`
> compatibility, the bounded deterministic playground, and the remaining
> deterministic Hub/Start/workflow-detail/Observatory parity graduation are on
> the released surface.
>
> This release is deterministic stabilization-and-hardening work unless separate
> real-runtime validation widens it. The published `0.8.8` baseline does not
> claim a calibration dashboard, API/webhook control plane, arbitrary
> code/plugin graph editing, a full persistent collaborative canvas layout, or a
> commercial runtime path.
>
> Current source tip still includes future-release candidates for Batch Runner,
...
## Released 0.8.8 stabilization-and-hardening train
...
| Surface | Released role | Boundary |
| --- | --- | --- |
...
| Batch Runner | `/batch`, deterministic row execution, live parsed-row table preview, cancel/retry/export, and row links back into Observatory. | Source-visible future-release candidate only until release docs and Gate 2 proof intentionally widen the train. |
| Versions | `/versions`, snapshot diffs/rollback, default selection, version-run provenance, Studio snapshot handoff, and saved-version execution. | Source-visible future-release candidate only; keep claims local-first and deterministic until graduated. |
...
### Required gates for this release train
...
- **Gate 1 is required for every PR:** docs/claim checks, lint/build/test slices
  ...
  changes land.
- **Gate 2 is required for release:** a fresh-environment new-user proof must
...
- Add a real OpenAI-compatible or coding-agent-CLI runtime proof only if release
  claims widen beyond the deterministic product-shell baseline. If cloud/API
...
- If Batch / Versions / API / Webhooks graduate from source-visible to released
  claims, extend Gate 2 to cover deterministic version execution, batch run
...
- **`future-release`** — implemented or approved candidate for a later coordinated release once package/docs/smoke gates move together
...
| --- | --- | --- | --- | --- | --- |
| Unified Hub / Studio / Playground / Observatory spine | `/`, `/hub`, `/studio`, `/playground`, `/observatory`, `/runs/<run-id>`, `/api/app-shell`, `/api/studio*`, `/api/drafts`, `/api/playground*`, and run/artifact read APIs | **`shipped`** | `0.8.8` | Released deterministic parity shell: Hub for first-run/templates, Studio for bounded drag-drop graph authoring, Playground for graph-linked execution and trace review, Observatory for run inspection, and the remaining Hub/Start/workflow-detail parity graduation. This is one progressively disclosed product, not separate B2C/B2B apps. | Preserve `/workbench` as a compatibility route, keep Gate 1 on PRs, and require release Gate 2 evidence for first-run Hub, template-to-Playground, Studio validate/save, Studio-to-Playground trace, Observatory drill-down, and deterministic baseline. |
...
| Editable workflow workbench / Studio graph IDE | `xrtm web --workflows-dir .xrtm/workflows`, `/`, `/hub`, `/runs`, `/studio`, `/workbench`, `/api/app-shell`, `/api/authoring/catalog`, `/api/studio*`, `/api/runs`, `/api/drafts`, `/api/runs/<run-id>/compare/<baseline-run-id>` | `/workbench` **`shipped`**; `/studio` graph IDE **`shipped`** | `0.8.1` baseline plus `0.8.6` Studio route | The released local WebUI is a React/TypeScript app shell backed by the local Python API and SQLite app-state. It can inspect runs, resume recent work, start from scratch/template/clone, author shared workflow fields, make safe node/edge/entry changes inside the built-in node catalog, validate, run, and compare against a selected baseline run. `/studio` is the primary bounded graph IDE with local node dragging, palette click/drag-to-canvas add-node, node/edge/workflow selection, edge create/remove, entry setting, contextual inspector, and validation/save/run through Studio APIs; `/workbench` remains compatibility. | Keep deterministic clean-room evidence covering WebUI smoke plus authoring → validate → run → compare, and keep release docs precise about remaining parallel-group/conditional-route limits. Do not break existing `/workbench` links while moving product language to Studio. |
...
package, release claims, and deterministic Gate 2 evidence move together.
...
2. Do **not** move future features into `README.md`, `docs/getting-started.md`, `docs/operator-runbook.md`, or `xrtm.org` release-pinned pages until the published package and `docs/release-command-contract.json` are updated together.
3. Graduation evidence for released docs must include the command-claim check plus deterministic clean-room acceptance from release artifacts (wheelhouse before publish, PyPI after publish). If the change touches local-model behavior, also require local-LLM clean-room evidence or an explicit defer note. Local-LLM release evidence should include the clean-room summary, benchmark artifacts, competition dry-run bundle, and GPU telemetry summary rather than a bare demo log.
...
7. Keep xrtm.org promotion in its own repo/todo; this page remains the xrtm command-level source of truth.
